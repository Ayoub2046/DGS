...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
  const { loginWithCredentials, settings, switchUserById } = useApp();
  ...
  const handleBiometricLogin = async () => {
    ...
    if (!hasEnrolledBio) {
      setInfoMsg(
        'No fingerprint is registered on this device yet. Please sign in with your Username and Password first, then register your fingerprint in your Security settings.'
      );
      return;
    }
    ...
    try {
      ...
      if (res.success && res.userId) {
        ...
        setError(res.error || 'Biometric authentication failed. Please enter your password.');
      }
    } catch (err: any) {
      setError(err?.message || 'Biometric sensor error. Please use username and password.');
    } finally {
            ...
            >
              ...
              <span>
                {isVerifyingBiometric
                  ...
                  : `Sign In with ${biometricInfo.label} (Requires Enrollment)`}
              </span>
            </button>
          )}
